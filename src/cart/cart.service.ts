...
export class CartService {
  ...
  async addToCart(subProduct: SubproductDto, idUser: string): Promise<DocumentData> {
    const cartDoc: DocumentData = await this.cartCollection.where('user', '==', idUser).where('isActive', '!=', false).get()
  ...
  async removeFromCart(subprod: SubproductDto, idUser: string): Promise<DocumentData> {
    const cartDoc: DocumentData = await this.cartCollection.where('user', '==', idUser).where('isActive', '!=', false).get()
    if (!cartDoc.empty) {
      const updatedCart = removeSubprodFromCart(subprod, cartDoc.docs[0].data())
      const cartId = cartDoc.docs[0].id
      const cartRef = this.cartCollection.doc(cartId)
  ...
  async updateSubprodQuantity(subprodQuantity: QuantityUpdateDto, idUser: string): Promise<DocumentData> {
    const cartDoc: DocumentData = await this.cartCollection.where('user', '==', idUser).where('isActive', '!=', false).get()
